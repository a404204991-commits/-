      ...
      try {
        ...
        const formattedCards = parsedData.map((item, idx) => {
          ...
          return {
            ...
            // 巧妙映射：将关联语汇注入到氛围和形态中，完美衔接原本的 AI 组合面板
            atmosphere: relTerms,
        ...
        setVocabularyCards([{
          id: 'v1', type: 'vocabulary', title: '先锋主义', category: '设计风格', category2: '进阶流派',
          description: '突破传统、追求创新与实验性。常见设计手法包括非对称布局、非常规材料运用、抽象化造型及大胆色彩碰撞。',
          relatedTerms: ['戏剧张力', '力量感', '理性'], relatedTermsReverse: ['咬合穿插', '几何构成'],
          atmosphere: ['戏剧张力', '力量感', '理性'], morphology: ['咬合穿插', '几何构成'],
            ...
            {isLoading ? (
              ...
              <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-6 space-y-6">
                {vocabularyCards.map(card => (
                  <div key={card.id} onClick={() => setSelectedCard(card)} className={`break-inside-avoid group cursor-pointer overflow-hidden rounded-[32px] border-2 transition-all duration-500 ${darkMode ? 'bg-zinc-900 border-zinc-800 hover:border-indigo-500 shadow-2xl shadow-black' : 'bg-white border-zinc-100 hover:border-black shadow-sm'} flex flex-col`}>
                    ...
                    <div className="p-6 flex-1 flex flex-col justify-between">
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); addToBoard(card); }} className="p-3 shrink-0 hover:bg-indigo-500 hover:text-white bg-zinc-50 dark:bg-zinc-800 rounded-2xl transition-all">
                          <Copy size={18} />
                        </button>
                      </div>
                        ...
                        {card.relatedTerms && card.relatedTerms.length > 0 ? (
                          <>
                            {card.relatedTerms.slice(0, 2).map((term, i) => (
                              <span key={i} className="truncate px-2.5 py-1 text-[10px] font-bold tracking-wider bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-lg max-w-[85px]">{term}</span>
                            ))}
                            {card.relatedTerms.length > 2 && <span className="px-2.5 py-1 text-[10px] font-bold tracking-wider bg-zinc-50 dark:bg-zinc-800 text-zinc-500 rounded-lg shrink-0">+{card.relatedTerms.length - 2}</span>}
                          </>
                        ) : (
...
function MoodBoard({ items, setItems, view, setView, darkMode, setIsAiAnalyzing, onOpenDetail, undo, redo, saveSnapshot, history, redoStack }) {
  ...
  useEffect(() => {
    const move = (e) => { mousePos.current = { x: e.clientX, y: e.clientY, mouseRawX: e.clientX, mouseRawY: e.clientY }; };
    const down = (e) => {
      if (isUserTyping()) return;
  ...
  return (
    <div className={`flex h-full relative ${isSpacePressed ? 'custom-grab' : ''}`} onContextMenu={(e) => { e.preventDefault(); setContextMenu({ x: e.clientX, y: e.clientY }); }}>
      ...
      >
            ...
            <BoardItem 
              ...
              onMove={(id, dx, dy) => {
                setItems(prev => {
                  ...
                  return prev.map(i => (i.boardId === id || (moveSelection && i.isSelected)) ? { 
                    ...i, x: Math.round(i.x + dx / view.scale), y: Math.round(i.y + dy / view.scale) 
                  } : i);
                });
              }}
              ...
              onSelect={(id, multi) => {
                setItems(prev => {
                  ...
                  const maxZ = Math.max(0, ...prev.map(i => i.zIndex || 0));
                  if (multi) {
                    const isNowSelected = !target.isSelected;
                    return prev.map(it => it.boardId === id 
                      ? { ...it, isSelected: isNowSelected, zIndex: isNowSelected ? maxZ + 1 : it.zIndex }
                      : it
                    );
                  }
                  if (target.isSelected) return prev.map(it => it.isSelected ? { ...it, zIndex: maxZ + 1 } : it);
                  return prev.map(it => it.boardId === id 
                    ? { ...it, isSelected: true, zIndex: maxZ + 1 }
                    : { ...it, isSelected: false }
                  );
                });
              }}
        ...
        <div className="absolute bottom-10 left-10 flex flex-col gap-2 z-30 transition-transform duration-300"><ControlButton icon={<Plus size={18}/>} onClick={() => addNewText()} darkMode={darkMode} title="插入语汇便签" /><ControlButton icon={<ZoomIn size={18}/>} onClick={() => setView(v => ({ ...v, scale: Math.min(v.scale * 1.2, 5) }))} darkMode={darkMode} /><ControlButton icon={<ZoomOut size={18}/>} onClick={() => setView(v => ({ ...v, scale: Math.max(v.scale / 1.2, 0.15) }))} darkMode={darkMode} /><ControlButton icon={<RotateCcw size={18}/>} onClick={() => setView({ x: 0, y: 0, scale: 1 })} darkMode={darkMode} /></div>
      </div>
      ...
      {contextMenu && (
        <div className={`fixed z-[200] w-52 py-2 rounded-2xl shadow-2xl border ${darkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-100'}`} style={{ left: contextMenu.x, top: contextMenu.y }} onClick={(e) => e.stopPropagation()}>
          ...
          <div className="my-1 border-t dark:border-zinc-800 opacity-50" />
          <MenuOption icon={<Copy size={16}/>} label="复制选中图层" onClick={() => { const sel = items.filter(i => i.isSelected); if(sel.length > 0) setInternalClipboard(JSON.parse(JSON.stringify(sel))); setContextMenu(null); }} darkMode={darkMode} />
          <MenuOption icon={<ClipboardPaste size={16}/>} label="在此处粘贴" onClick={() => { 
      ...
      <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={(e) => { const f = e.target.files[0]; if (f) { const r = new FileReader(); r.onload = (ev) => addNewImage(ev.target.result, mousePos.current.mouseRawX, mousePos.current.mouseRawY); r.readAsDataURL(f); } }} />
      ...
      <div className={`w-80 border-l p-8 flex flex-col ${darkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-100'} select-auto`} onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <section><h4 className="text-[10px] font-black text-zinc-400 mb-4 uppercase tracking-widest">AI 提示词</h4><div className={`p-5 rounded-[24px] border-2 border-dashed min-h-[140px] text-[11px] leading-relaxed italic ${darkMode ? 'bg-zinc-950 border-zinc-800 text-zinc-500' : 'bg-zinc-50 border-zinc-200 text-zinc-400'}`}>{items.some(i => i.isSelected) ? `室内写实摄影, ${[...new Set(items.filter(i => i.isSelected).flatMap(i => [...(i.atmosphere || []), ...(i.morphology || [])]))].join(', ')}...` : "指令预览..."}</div>{items.some(i => i.isSelected) && <button className="w-full mt-6 py-4 bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-indigo-500/20 flex items-center justify-center gap-2 transition-transform hover:scale-105 active:scale-95"><Wand2 size={16}/> 生成空间草图</button>}</section>
        </div>
      </div>
    </div>
  );
...
function BoardItem({ item, darkMode, onMove, onMoveEnd, onSelect, onDoubleClick, onTitleChange, onTextChange, onSizeUpdate }) {
  ...
  return (
    <div ref={containerRef} style={outerStyle} onMouseDown={handleMouseDown} onDoubleClick={() => onDoubleClick(item.boardId)} className={isDragging ? 'custom-grabbing' : 'custom-grab'}>
      <div className={`transition-all duration-300 transform-gpu ${item.type === 'text' ? 'w-fit' : 'w-52'} rounded-[32px] p-2 border-4 ${item.isSelected ? 'border-indigo-600 ring-8 ring-indigo-500/15 scale-[1.03] shadow-2xl' : 'border-transparent shadow-xl'} ${darkMode ? 'bg-zinc-900' : 'bg-white'}`}>
        <div className="relative group flex flex-col items-center">
          {item.type !== 'text' && <><div className="absolute top-3 left-3 px-2 py-0.5 bg-black/50 backdrop-blur-md text-[8px] font-bold text-white rounded uppercase tracking-tighter z-10">{item.category || '自定义'}</div><div className="aspect-[4/5] w-full bg-zinc-100 dark:bg-zinc-800 rounded-[24px] overflow-hidden relative flex items-center justify-center">{item.mainImage ? <img src={item.mainImage} className="w-full h-full object-cover pointer-events-none" alt="" /> : <ImageOff className="opacity-10" />}</div></>}
          <div className={`flex flex-col justify-center items-center w-full ${item.type === 'text' ? 'p-1' : 'p-4'}`}>
            {item.type === 'text' ? (
              ...
              <div className="flex flex-col gap-1 w-full text-center">
                ...
                {!item.isAiAnalyzed && <div className="text-[8px] text-indigo-500 font-bold tracking-widest uppercase animate-pulse">Pending...</div>}
              </div>
            )}
          </div>
        </div>
...
function DetailModal({ card, onClose, darkMode, onAddToBoard }) {
  return (
    <div onClick={onClose} className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-md bg-black/80">
      <div onClick={(e) => e.stopPropagation()} className={`relative w-full max-w-5xl max-h-[85vh] overflow-y-auto rounded-[48px] flex flex-col md:flex-row shadow-2xl ${darkMode ? 'bg-zinc-900 border border-zinc-800' : 'bg-white'}`}>
        <button className="absolute right-8 top-8 p-3 hover:bg-black/10 rounded-full z-20 transition-colors" onClick={onClose}><X size={24}/></button>
        <div className="md:w-[50%] bg-zinc-100 dark:bg-zinc-950 flex items-center justify-center overflow-hidden border-r border-zinc-200 dark:border-zinc-800 relative min-h-[40vh] md:min-h-auto">
          {card.mainImage ? (
            <img src={card.mainImage} className="w-full h-full object-cover" alt="" />
          ) : (
             <ImageOff className="text-zinc-300 dark:text-zinc-700" size={64} />
          )}
        </div>
        <div className="md:w-[50%] p-10 md:p-12 flex flex-col">
          <div className="flex gap-2 mb-6">
             <span className="px-3 py-1.5 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 text-[10px] font-black rounded-lg uppercase tracking-widest">{card.category}</span>
             {card.category2 && <span className="px-3 py-1.5 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 text-[10px] font-black rounded-lg uppercase tracking-widest">{card.category2}</span>}
          </div>
          <h2 className="text-3xl md:text-4xl font-black tracking-tighter mb-6">{card.title}</h2>
          <p className="text-sm opacity-80 leading-relaxed mb-10 text-justify whitespace-pre-wrap">{card.description}</p>
          
          {/* 新增的关联图谱呈现区 */}
          <div className="mb-10 space-y-6">
             {card.relatedTerms && card.relatedTerms.length > 0 && (
                <div>
                  <h4 className="text-xs font-black text-zinc-400 uppercase tracking-widest mb-3 flex items-center"><BookOpen size={14} className="mr-2"/> 正向关联语汇</h4>
                  <div className="flex flex-wrap gap-2">
                     {card.relatedTerms.map((term, i) => (
                        <span key={i} className="px-3 py-1.5 bg-zinc-100 dark:bg-zinc-800 text-xs font-bold text-zinc-600 dark:text-zinc-300 rounded-xl">{term}</span>
                     ))}
                  </div>
                </div>
             )}
             {card.relatedTermsReverse && card.relatedTermsReverse.length > 0 && (
                <div>
                  <h4 className="text-xs font-black text-zinc-400 uppercase tracking-widest mb-3 flex items-center"><BookOpen size={14} className="mr-2"/> 被关联语汇</h4>
                  <div className="flex flex-wrap gap-2">
                     {card.relatedTermsReverse.map((term, i) => (
                        <span key={i} className="px-3 py-1.5 bg-zinc-100 dark:bg-zinc-800 text-xs font-bold text-zinc-600 dark:text-zinc-300 rounded-xl">{term}</span>
                     ))}
                  </div>
                </div>
          ...
          <div className="mt-auto pt-6 border-t border-zinc-100 dark:border-zinc-800">
            <button onClick={() => { onAddToBoard(card); onClose(); }} className="w-full py-5 rounded-2xl text-[12px] font-black uppercase tracking-widest bg-indigo-600 text-white hover:bg-indigo-700 shadow-xl shadow-indigo-500/20 transition-all flex justify-center items-center gap-2">
              <Plus size={18} /> 加入情绪板推敲
            </button>
          </div>
        </div>
      </div>
      ...
      <div onClick={(e) => e.stopPropagation()} className={`relative w-full max-w-6xl h-[85vh] overflow-hidden rounded-[48px] flex flex-col md:flex-row shadow-2xl ${darkMode ? 'bg-zinc-900 border border-zinc-800' : 'bg-white'}`}>
        ...
        <div className="md:w-[45%] p-12 overflow-y-auto border-l dark:border-zinc-800">
          ...
          <div className="space-y-10">
            ...
            <div>
              <label className="text-[10px] font-black text-zinc-400 mb-3 block uppercase">空间氛围 / 正向关联</label>
            ...
            <div>
              ...
              {!item.isLibraryItem && <div className="flex gap-2"><input placeholder="添加关联..." className="flex-1 bg-transparent border-b text-xs py-1 outline-none focus:border-indigo-500" onKeyDown={(e) => { if (e.key === 'Enter') { const v = e.target.value; if(v) { onUpdate({ morphology: [...(item.morphology || []), v] }); e.target.value = ''; } } }} /></div>}
            </div>
          </div>
        </div>
      </div>
